...
Transaction `0x762bcc…` is the contrast in the same token: a single **980.25** BUIDL `Transfer` between allowlisted addresses: movement, not mint workflow. Not every dense BUIDL row is the same business event.
